...
# 由于网易云音乐歌曲评论采取AJAX填充的方式，所以在HTML上爬不到，需要调用评论API，
# 而API进行了加密处理，下面是解决方法
...
def rsaEncrypt(text,pubKey,modulus):
    # text=text[::-1]
    # rs= int(text.encode('hex'),16) ** int(pubKey,16) % int(modules,16)

    text1=text[::-1]
    text2=text1.encode('utf-8')
    rs= int(binascii.hexlify(text2),16) ** int(pubKey,16) % int(modulus,16)
...
# 通过第三方渠道获取网易云音乐的所有歌曲ID
def getSongIdListBy3Party():
    pageMax=1 #要爬的页数，可以根据需求选择
    songIdList=[]
    for page in range(pageMax):
        ...
        # print soup
        aList=soup.findAll('a',sttrs={'target':'_blank'})
        for a in aList:
            songId=a['href'].split('=')[1]
            songIdList.append(songId)
    return songIdList

# 从官网的 发现》歌单 页面爬取网易云音乐的所有歌曲ID
def getSongIdList():
    pageMax=3
    songIdList=[]
...
def setSongInfo(song):
    ...
    # 去掉歌曲名称后面（）内的字，如果不想去掉，可以注释下面三行代码
    # index=name.find('(')
    # if index>0:
    #     name=name[0:index]
    song.name=name
...
def getSongList():
    ...
    for id in songIdList:
        ...
        if None!=song:
            ...
            sname=song.name.decode('utf-8')

            print("成功匹配一首{名称：",sname,"-",song.singer,",评论数:",song.commentCount,"}")
    print("##爬取完成，符合条件的共计："+str(len(songList))+"首")
    return songList
